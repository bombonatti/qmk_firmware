...
*/
 ...
 [BASE] = LAYOUT_60_ansi( /* Base */
    ...
    KC_LCTL,          LGUI_T(KC_NUM_LOCK), KC_LALT,                   KC_SPC,             KC_RALT, LT(FN1, KC_LEFT), LT(FN2, KC_DOWN), RCTL_T(KC_RGHT)
),
  /*
  * Layer FN1
  * ,-----------------------------------------------------------------------------------------.
  ...
  * Layer FN2
  * ,--------------------------------------------------------------------------------------------------.
  * |BT_UNPAIR| BT1 | BT2 | BT3 | BT4 | MREC1 | MPLA1 | MRECSTOP | F8 |LEDTG|LEDI+|LEDPV|LEDNX|  Bksp  |
  * |--------------------------------------------------------------------------------------------------+
  * | Tab     |  q  | UP  |  e  |  r  |  t    |  y    |  u       |  i  |  o  | PS | HOME| END |   \    |
  * |--------------------------------------------------------------------------------------------------+
  * | Esc     |LEFT |DOWN |RIGHT|  f  |  g    |  h    |  j       |  k  |  l  | PGUP|PGDN |    Enter    |
 ...
 [FN2] = LAYOUT_60_ansi( /* FN2 */
    KC_AP2_BT_UNPAIR, KC_AP2_BT1, KC_AP2_BT2, KC_AP2_BT3, KC_AP2_BT4, DYN_REC_START1, DYN_MACRO_PLAY1, DYN_REC_STOP, _______, KC_AP_RGB_MOD, KC_AP_RGB_TOG, KC_AP_RGB_VAD, KC_AP_RGB_VAI, _______,
    _______, _______,    KC_UP,      _______,    _______,    _______, _______, _______, _______, _______, KC_PSCR, KC_HOME, KC_END,  _______,
    _______, KC_LEFT,    KC_DOWN,    KC_RGHT,    _______,    _______, _______, _______, _______, _______, KC_PGUP, KC_PGDN, _______,
...
ap2_led_t color[]= { {.p.red = 0xFF, .p.green = 0xFF, .p.blue = 0xAA, .p.alpha = 0xFF} //white no alpha
                    ,{.p.red = 0xFF, .p.green = 0xFF, .p.blue = 0x99, .p.alpha = 0xFF} //white
                    ,{.p.red = 0xFF, .p.green = 0xFF, .p.blue = 0x99, .p.alpha = 0xFF} //white
                    ,{.p.red = 0xFF, .p.green = 0x00, .p.blue = 0x00, .p.alpha = 0xFF} //red
                };
        ...
        case NLOCK:
            ...
            for(i=0; i<MATRIX_ROWS; i++) {
                for(j=0; j<MATRIX_COLS; j++) {
                    if(keymaps[get_highest_layer(state)][i][j]!=_______)
                        ap2_led_mask_set_key(i,j, color[get_highest_layer(state)]);
                }
            }
            break;
        default:
    ...
    for(i=0; i<MATRIX_ROWS; i++) {
        for(j=0; j<MATRIX_COLS; j++) {
            if(keymaps[MASKSET][i][j]!=_______)
                ap2_led_sticky_set_key(i,j, color[BASE]);
        }
    }
